...
int pmp_set(unsigned int index, ulong_t cfg_val, ulong_t addr_val)
{
	ulong_t reg_val;
	ulong_t shift, mask;
	int pmpcfg_csr;
	...
	addr_val = addr_val >> PMP_SHIFT_ADDR;
...
int pmp_get(unsigned int index, ulong_t *cfg_val, ulong_t *addr_val)
{
	...
	*addr_val = csr_read_enum(pmpaddr_csr) << PMP_SHIFT_ADDR;

	return 0;
}

void pmp_clear_config(void)
{
...
void pmp_print(unsigned int index)
{
	ulong_t cfg_val;
	ulong_t addr_val;
...
void init_user_accesses(struct k_thread *thread)
{
	 ...
	 *
	 * If stack guards are enabled, the thread stacks will not be aligned,
	 * and the TOR addressing mode must be used. This requires an additional
	 * PMP register.
	 *
	 *  +=========+ <--  0x0
	 *  +---------+ <--  pmpaddr0 [NAPOT, RX]
